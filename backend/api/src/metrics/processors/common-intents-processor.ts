...
@Injectable()
export class CommonIntentsProcessor extends BaseMetricProcessor {
  ...
  async process(query: MetricQuery): Promise<MetricResult> {
    ...
    const logData = await this.logRepository
      ...
      .select(
        `DATE_TRUNC('${timeBucket}', log.timestamp) as timestamp, log.query->>'intent' as value, COUNT(*) as count`,
      )
      .where('log.timestamp BETWEEN :startDate AND :endDate', {
        startDate,
        endDate,
      })
      .andWhere("log.query->>'intent' IS NOT NULL")
      .groupBy(
        `DATE_TRUNC('${timeBucket}', log.timestamp), log.query->>'intent'`,
      )
      .orderBy('count', 'DESC')
